...
public class SnowballFightSetup {
    public static void finishSetup(Player player, Integer arenaid, Main main) {
        ...
        main.configManager.getArena(arenaid).set("arena.mini_games.snowball_fight.basic.enabled", true);
        main.configManager.saveArena(arenaid);
        main.configManager.getArena(arenaid).set("arena.mini_games.snowball_fight.basic.death_block",
                main.configManager.getSettings().getString("game.global.default_death_block"));
        main.configManager.saveArena(arenaid);
        main.configManager.reloadArena(arenaid);

        StringUtils.sendMessage(player, player.getName(), CacheManager.Language.GLOBAL_SUCCESS_MINI_GAME_SET.replace("{game}", "Snowball Fight"));
    }
}
